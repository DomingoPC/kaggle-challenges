...
normality <- df.train %>%    
  summarise(across(     
    where(is.numeric) & !Calories, test_normality_ks 
  )) %>%    
...
for (var in vars_need_transform) {
  if(all( df.train[[var]] > 0, na.rm = T )) {
    lambda <- BoxCox.lambda(df.train %>% sample_n(1e5) %>% select(all_of(var)))
    df.train_trans[[paste0(var)]] <- BoxCox(df.train[[var]], lambda)
  }
}
...
```{r}
numeric_vars <- setdiff(df.train %>% select_if(is.numeric) %>% names(),
                        'Calories')
df.train <- df.train %>% 
  mutate(
    across(
      where(is.numeric) & !Calories, 
      ~(scale(.) %>% as.vector())
    )
  )
...
### Variable Transforms: Body_Temp

```{r}
# Exponentiate Body_Temp
exponentiate_body_temp <- function(data) {
  return(
    exp( data$Body_Temp )
  )
}

df.train['exp_Body_Temp'] <- exponentiate_body_temp(df.train)
df.val['exp_Body_Temp'] <- exponentiate_body_temp(df.val)
...
predict_test_clusters <- function(df_test, kmeans_model) {
  # # Scale test features using training parameters
  # test_features <- df_test %>% select(-Calories)  # Exclude target
  # test_scaled <- scale(test_features, 
  #                     center = train_means[-which(names(train_means) == "Calories")],
  #                     scale = train_sds[-which(names(train_sds) == "Calories")])
  # 
  # Find closest cluster center (using only feature dimensions)
  feature_centers <- kmeans_model$centers[, -which(colnames(kmeans_model$centers) == "Calories")]
  
  distances <- apply(df_test, 1, function(x) {
    apply(feature_centers, 1, function(center) {
      sqrt(sum((x - center)^2))
    })
  })
